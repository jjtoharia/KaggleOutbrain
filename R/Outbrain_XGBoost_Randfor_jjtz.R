...
# --------------------------------------------------------
G_b_DEBUG <- TRUE # Reducimos todo para hacer pruebas más rápido
NUM_BLOQUES <- 16
# --------------------------------------------------------
gc()
# ------------------------------------------------------------------------------------
...
for(numBatch in 1:NUM_BLOQUES)
{
  ...
  xgb_mi_version <- 300 + xgb_preps[[1]] # Esto viene de xgb_prep_datos (indica cambios en las "features" seleccionadas)
  ...
  xgb_eta = 0 # 0.3 [0.01 - 0.2] step size of each boosting step (si baja, afina mejor pero habrá que subir nround)
  xgb_max_depth = 15 # maximum depth of the tree (demasiado alto => overfitting)
  xgb_nround = 1 # max number of iterations
  xgb_subsample = 0.75 # (muestreo aleatorio por filas si es < 1, para intentar evitar overfitting)
  xgb_colsample_bytree = 0.5 # (muestreo aleatorio por columnas si es < 1, para intentar evitar overfitting)
  ...
  # xgb_num_class = 12
  xgb_scale_pos_weight = mi_sum_neg / mi_sum_pos # == 4.164079
  xgb_min_child_weight = 0.25 # 1 [0.1 - 10]
  ...
  {
    xgb_s_tipo_modelo = "XGB"   # XGBoost
    xgb_num_parallel_tree = 1
  } else {
    xgb_s_tipo_modelo = "XGRF"  # Random Forest
    xgb_nround = 1
  }
  # ===============  XGB params - END  ===============
  if(G_b_DEBUG & xgb_nround > 1)  xgb_nround = 10
  ...
  xgb_filename <- paste0(paste(xgb_s_tipo_modelo,
                               ...
                               xgb_eta, xgb_max_depth, xgb_subsample, xgb_colsample_bytree, xgb_eval_metric, xgb_early.stop.round, round(xgb_scale_pos_weight,2), xgb_min_child_weight, xgb_gamma, xgb_num_parallel_tree,
  ...
  if(xgb_nround > 1)
  {
    ...
    xgb_cv <- xgb.cv(data = X, label = y, missing = NA,
                     ...
                     objective = xgb_objective, eta = xgb_eta, max_depth = xgb_max_depth, subsample = xgb_subsample, colsample_bytree = xgb_colsample_bytree, eval_metric = xgb_eval_metric, early.stop.round = xgb_early.stop.round,
...
predict_testset <- function()
{
  ...
  for(numBatch in 1:NUM_BLOQUES)
  {
    ...
    for(i in 1:NUM_BLOQUES)
    {
      print(paste0('XGB Predicting ', str_pad(numBatch, 3, "left" ,"0"), ' (modelo ', i, ' de ', NUM_BLOQUES, '): ', xgb_modelos[i],'...'))
